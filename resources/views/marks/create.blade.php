...
@php
    ...
    $canSave = !$final_marks_submitted && count($exams) > 0;
    $canSubmitFinal = !$final_marks_submitted && count($exams) > 0 && $academic_setting['marks_submission_status'] == 'on';
@endphp
...
<div class="container">
    <div class="row justify-content-start">
        ...
        <div class="col-xs-11 col-sm-11 col-md-11 col-lg-10 col-xl-10 col-xxl-10">
            <div class="row pt-2">
                <div class="col ps-4">
                    <h1 class="display-6 mb-3"><i class="bi bi-cloud-sun"></i> Give Marks</h1>
                    ...
                    @if ($academic_setting['marks_submission_status'] == 'on')
                        <p class="text-primary"><i class="bi bi-exclamation-diamond-fill me-2"></i>Marks Submission Window is open now.</p>
                    @endif

                    <p class="text-primary">
                        <i class="bi bi-grid-3x3-gap-fill me-2"></i>
                        Formula: WWPS=(WW Score/WW High)*100, PTPS=(PT Score/PT High)*100, QAPS=(Exam Score/Exam High)*100,
                        IG=(WWPS*0.40)+(PTPS*0.40)+(QAPS*0.20), Final Grade=rounded IG.
                    </p>

                    @if ($final_marks_submitted)
                        <p class="text-success"><i class="bi bi-exclamation-diamond-fill me-2"></i>Marks are submitted.</p>
                    @endif
                        ...
                        <div class="col-3 mt-3">
                            <a type="button"
                               ...
                               onclick="return confirm('Are you sure, you want to submit final marks?')">
                                <i class="bi bi-check2"></i> Submit Final Marks
                            </a>
                        </div>
                    @endif
                    ...
                    <form action="{{ route('course.mark.store') }}" method="POST">
                                        ...
                                        <tbody>
                                            @foreach ($sectionStudents as $sectionStudent)
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input type="number" class="form-control final-grade-input" readonly></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                        ...
                        @if (!$canSave)
                            <div class="col-5">
                                @if ($final_marks_submitted)
                                    <p class="text-success"><i class="bi bi-exclamation-diamond-fill me-2"></i>You have submitted Final Marks <i class="bi bi-stars"></i>.</p>
                                @else
                                    <p class="text-primary"><i class="bi bi-exclamation-diamond-fill me-2"></i>Create Exam to give marks.</p>
                                @endif
                            </div>
                        @endif
                    </form>
...
(function () {
    ...
    function calculateRow(row) {
        var wwScore = toNumber(row.querySelector('.category-score[data-category="written"]').value);
        var wwHighest = toNumber(row.querySelector('.category-high[data-category="written"]').value);
        var ptScore = toNumber(row.querySelector('.category-score[data-category="performance"]').value);
        var ptHighest = toNumber(row.querySelector('.category-high[data-category="performance"]').value);
        var qaScore = toNumber(row.querySelector('.category-score[data-category="quarterly"]').value);
        var qaHighest = toNumber(row.querySelector('.category-high[data-category="quarterly"]').value);

        var wwPS = wwHighest > 0 ? (wwScore / wwHighest) * 100 : 0;
        var ptPS = ptHighest > 0 ? (ptScore / ptHighest) * 100 : 0;
        var qaPS = qaHighest > 0 ? (qaScore / qaHighest) * 100 : 0;

        var IG = (wwPS * 0.40) + (ptPS * 0.40) + (qaPS * 0.20);
        var finalGrade = Math.round(IG);

        row.querySelector('.final-grade-input').value = finalGrade;

        var wwStoreInput = row.querySelector('.store-score-input[data-category="written"]');
        var ptStoreInput = row.querySelector('.store-score-input[data-category="performance"]');
        var qaStoreInput = row.querySelector('.store-score-input[data-category="quarterly"]');
    ...
    var rows = document.querySelectorAll('#deped-grading-table tbody tr');
    rows.forEach(function (row) {
        var inputs = row.querySelectorAll('.category-score, .category-high');
